...
class CoordinateConversionTests: XCTestCase {
  ...
  func testTEME_to_ECEF_atJ2000() throws {
    ...
    let temeMagnitude = sqrt(
      temePosition.x * temePosition.x + temePosition.y * temePosition.y + temePosition.z
        * temePosition.z)
    let ecefMagnitude = sqrt(
      ecefPosition.x * ecefPosition.x + ecefPosition.y * ecefPosition.y + ecefPosition.z
        * ecefPosition.z)
  ...
  func testCoordinateConversion_DateDependence() throws {
    ...
    let difference = sqrt(
      pow(ecef1.x - ecef2.x, 2) + pow(ecef1.y - ecef2.y, 2) + pow(ecef1.z - ecef2.z, 2)
    )
  ...
  func testGeodetic_AltitudeCalculation() throws {
    ...
    for (radius, description) in testCases {
      ...
      XCTAssertEqual(
        geodetic.altitude, radius - 6378.137, accuracy: 10.0,
