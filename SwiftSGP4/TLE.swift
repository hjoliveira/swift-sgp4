...
public struct TLE {
  ...
  public init(name: String, lineOne: String, lineTwo: String) throws {
    ...
    func parseScientificNotation(_ str: String) -> Double {
      ...
      let exponentStr = String(
        trimmed.suffix(from: trimmed.index(trimmed.startIndex, offsetBy: expSignPos)))
    ...
    let satNumber1 = trimmedSubstring(
      str: lineOne, location: TLE1_COL_NORADNUM, length: TLE1_LEN_NORADNUM)
    let satNumber2 = trimmedSubstring(
      str: lineTwo, location: TLE2_COL_NORADNUM, length: TLE2_LEN_NORADNUM)
    ...
    // line 1
    self.intDesignator = trimmedSubstring(
      str: lineOne, location: TLE1_COL_INTLDESC_A,
      length: TLE1_LEN_INTLDESC_A + TLE1_LEN_INTLDESC_B + TLE1_LEN_INTLDESC_C)

    guard
      var year = Int(
        trimmedSubstring(str: lineOne, location: TLE1_COL_EPOCH_A, length: TLE1_LEN_EPOCH_A))
    else {
      throw TLEError.invalidElement("Invalid year")
    }

    guard
      let doubleDay = Double(
        trimmedSubstring(str: lineOne, location: TLE1_COL_EPOCH_B, length: TLE1_LEN_EPOCH_B))
    else {
    ...
    self.meanMotionDdt6 = parseScientificNotation(
      trimmedSubstring(
        str: lineOne, location: TLE1_COL_MEANMOTIONDDT6, length: TLE1_LEN_MEANMOTIONDDT6))

    self.bstar = parseScientificNotation(
      trimmedSubstring(str: lineOne, location: TLE1_COL_BSTAR, length: TLE1_LEN_BSTAR))

    // line 2
    self.inclination = parseDouble(
      trimmedSubstring(str: lineTwo, location: TLE2_COL_INCLINATION, length: TLE2_LEN_INCLINATION))

    self.rightAscendingNode = parseDouble(
      trimmedSubstring(str: lineTwo, location: TLE2_COL_RAASCENDNODE, length: TLE2_LEN_RAASCENDNODE)
    )
    ...
    self.eccentricity =
      parseDouble(
        trimmedSubstring(
          str: lineTwo, location: TLE2_COL_ECCENTRICITY, length: TLE2_LEN_ECCENTRICITY))
      / 10_000_000.0

    self.argumentPerigee = parseDouble(
      trimmedSubstring(str: lineTwo, location: TLE2_COL_ARGPERIGEE, length: TLE2_LEN_ARGPERIGEE))

    self.meanAnomaly = parseDouble(
      trimmedSubstring(str: lineTwo, location: TLE2_COL_MEANANOMALY, length: TLE2_LEN_MEANANOMALY))

    self.meanMotion = parseDouble(
      trimmedSubstring(str: lineTwo, location: TLE2_COL_MEANMOTION, length: TLE2_LEN_MEANMOTION))

    if let orbitNumber = Int(
      trimmedSubstring(str: lineTwo, location: TLE2_COL_REVATEPOCH, length: TLE2_LEN_REVATEPOCH))
    {
