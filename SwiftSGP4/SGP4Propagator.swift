  ...
  private func initializeNearEarth() throws {
    ...
    self.c4 =
      ...
      * (eta * (2.0 + 0.5 * etasq) + e0 * (0.5 + 2.0 * etasq) - j2 * tsi / (aodp * psisq)
        * (-3.0 * x3thm1 * (1.0 - 2.0 * eeta + etasq * (1.5 - 0.5 * eeta)) + 0.75 * x1mth2
          * (2.0 * etasq - eeta * (1.0 + etasq)) * cos(2.0 * omega0)))
    ...
    omgdot =
      -0.5 * temp1 * x1m5th + 0.0625 * temp2 * (7.0 - 114.0 * theta2 + 395.0 * theta4) + temp3
    ...
    if perige < 220.0 {
      ...
      t5cof = 0.2 * (3.0 * d4 + 12.0 * c1 * d3 + 6.0 * d2 * d2 + 15.0 * cc1sq * (2.0 * d2 + cc1sq))
    }
  }
